...
class ElectronicDensity(object):
    def __init__(self, molecule):
        ...
        self.atomic_values = {
            "Titles": ['Atomic Number', 'Atomic weight', 'a1', 'w1', 'a2', 'w2', 'a3', 'w3', 'a4', 'w4', 'a5', 'w5', 'a6','w6','a7','w7',
            ...
            "C": [6, 12, 0.356462048751, 0.481662583478, 6.74589838364, 0.165621922856, 20.0053616858, 0.126630091695,
                  97.0508229097, 0.0169326970446, 0.122734548099, 0.209152704926,1,0,1,0, 2, 4 / 9, 0, 0, 0, 2 / 9, 0, 0],
            "N": [7, 14, 0.558690868187, 0.428044393139, 26.3091078048, 0.119405132277, 132.716385327, 0.0149293216485,
                  0.213410401209, 0.306224338493, 8.67117752141, 0.131396814443,1,0,1,0, 2, 0.396825, 0, 0, 0.31746, 0, 0, 0],
            "O": [8, 16, 0.764949764007, 0.462949674244, 36.3980482778, 0.0972634518752, 178.279439955, 0.0124323059053,
                  0.258345655165, 0.306747136544, 12.1009178518, 0.120607431432, 1,0,1,0, 2, 0.416666, 0, 0, 1 / 3, 0, 0, 0],
            "F": [9, 19, 0.953231108644, 0.521113130773, 0.288290993131, 0.272227674853, 14.3826569656, 0.102468771030,
                  44.6764228163, 0.0927110692516, 223.415128313, 0.0114793540601,1,0,1,0, 2, 0, 7 / 9, 0, 0, 0, 0, 0],
            ...
            "P": [15, 31, 0.361451144211E+01, 0.478831971088E+00, 0.203232522123E+00, 0.407091911330E+00,
                  ...
                  0.241672185606E+03, 0.270948640067E-01, 0.140780726438E+04, 0.170645174514E-02,1,0,1,0, 10, 0, 1 / 3, 0, 0,
                  0, 0, 0],
            "S": [16, 32, 0.432801783704E+01, 0.450261917524E+00, 0.234919053755E+00, 0.443771174997E+00,
                  0.748183927869E+02, 0.797906413585E-01,
                  0.281418786843E+03, 0.246258047332E-01, 0.163076922864E+04, 0.155046138826E-02, 1,0,1,0,10, 0, 0.3375, 0, 0,
                  0.0375, 0, 0],
            "Cl": [17, 35, 0.512735107009E+01, 0.423533567847E+00, 0.274578692637E+00, 0.477496012097E+00,
                   0.856745844595E+02, 0.745409754409E-01, 0.319382224252E+03, 0.229544216482E-01, 1, 0, 1,0,1,0,10, 0, 0.4117,
                   0, 0, 0, 0, 0],
    ...
    def _weights(self, element):
        ...
        weights = np.array([atomic_values[element][3], atomic_values[element][5], atomic_values[element][7],
                                 atomic_values[element][9], atomic_values[element][11],atomic_values[element][13],atomic_values[element][15]])
        return weights

    def _valence_weights(self,element):
        atomic_values=self.atomic_values
        valence_weights=np.array([atomic_values[element][17],atomic_values[element][18],atomic_values[element][19],
                                  atomic_values[element][20],atomic_values[element][21],atomic_values[element][22],atomic_values[element][23]])
    ...
    def density_val(self, pos, center=None):
        ...
        for i in range(len(sym_l)):
            ...
            rhoi = 0
            num_weights=self.num_weights(sym_l[i])
    ...
    def valence_dens(self,pos,center=None):
        ...
        for i in range(len(sym_l)):
            ...
            for j in range(num_weights):
                if atomic_values[sym_l[i]][2 * (j + 1) + 1] > 0:
                    norm = (2 * atomic_values[sym_l[i]][2 * (j + 1)] / np.pi) ** (3 / 2)
                    rhoi = rhoi + norm * atomic_values[sym_l[i]][j+17] * np.exp(
                        -2 * r * atomic_values[sym_l[i]][2 * (j + 1)])
            rho = rho + (atomic_values[sym_l[i]][0]) * rhoi
